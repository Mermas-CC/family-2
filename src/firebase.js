...
import { getFirestore, collection, addDoc, doc, getDocs, query, where, updateDoc, arrayUnion, getDoc, orderBy, limit, startAfter } from "firebase/firestore";
...
export const addPhoto = async (groupId, userId, title, description, date, imageUrl) => {
...
export const getPhotos = async (groupId, lastVisible) => {
    let q = query(
    ...
    const newLastVisible = photoSnapshot.docs[photoSnapshot.docs.length - 1];
    return { photoList, newLastVisible };
};
